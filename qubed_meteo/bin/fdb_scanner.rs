...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Parse selector.
    // ------------------------------------------------------------------
    let selector_map = selector_to_json(&args.selector)?;
    ...
    let (scan_start, scan_end, chunk_size) = if args.full {
        ...
        let span = (dataset_end - dataset_start).num_days().min(default_chunk_days).max(1);
        ...
        (dataset_start, dataset_end, Duration::days(span))
    } else {
    ...
    // ------------------------------------------------------------------
    // Merge with any existing persisted qube, report new data, and save.
    // ------------------------------------------------------------------
    let existing_qube = load_qube(&target_filepath);
    println!("Scanned {} leaves on this run.", accumulated_qube.datacube_count());

    // POST the full accumulated qube to the API before merging (to avoid consuming it).
    match post_qube(&http_client, &args.api, &accumulated_qube, &secret) {
        Ok(_) => println!("Final upload to API succeeded."),
        Err(e) => eprintln!("Warning: final API upload failed: {}", e),
    }
